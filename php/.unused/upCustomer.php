...
if(isset($_POST['submit'])){
    if(empty($_POST['customer'])) echo "Error choose one";
    else{
        ...
        if(isset($_POST['update'])){
            ...
            $id = $_POST['id'];
            if($id) {
                $respon = @mysqli_query($dbc,"UPDATE Customer SET Customer_ID = '$id'");
                if(!$respon) {
            ...
            if($f_name) {
                $respon = @mysqli_query($dbc,"UPDATE Customer SET First_Name = '$f_name'");
                if(!$respon) {
            ...
            if($l_name) {
                $respon = @mysqli_query($dbc,"UPDATE Customer SET Last_Name = '$l_name'");
                if(!$respon) {
            ...
            $irs = $_POST['irs'];
            if($irs) {
                $respon = @mysqli_query($dbc,"UPDATE Customer SET IRS = '$irs'");
                if(!$respon) {
            ...
            $ssn = $_POST['ssn'];
            if($ssn) {
                $respon = @mysqli_query($dbc,"UPDATE Customer SET Social_Security_Number = '$ssn'");
                if(!$respon) {
            ...
            $d_lic = $_POST['d_lic'];
            if($d_lic) {
                $respon = @mysqli_query($dbc,"UPDATE Customer SET Driver_License = '$d_lic'");
                if(!$respon) {
            ...
            $street = $_POST['street'];
            if($street) {
                $respon = @mysqli_query($dbc,"UPDATE Customer SET Street = '$street'");
                if(!$respon) {
            ...
            if($street_number) {
                $respon = @mysqli_query($dbc,"UPDATE Customer SET Street_Number = '$street_number'");
                if(!$respon) {
            ...
            $city = $_POST['city'];
            if($city) {
                $respon = @mysqli_query($dbc,"UPDATE Customer SET City = '$city'");
                if(!$respon) {
...
?>
        <tr><td><input name="update" type="submit" id="update" value="Update" /></td>
<td><input name="button" type="button" value="Choose other Customer" onclick="window.location.href='http://localhost/project/php/upCustomer.php'"/></td></tr>
<?php
...
?>
    <div id="form">
    <form action="http://localhost/project/php/upCustomer.php" method="post">
